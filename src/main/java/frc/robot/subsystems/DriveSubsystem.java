...
public class DriveSubsystem extends SubsystemBase {
	...
	public ChassisSpeeds computeChassisSpeeds(double axFwd, double axStrafe, double axRot) {
		...
		double speedFwd = xLimiter
				.calculate(Math.pow(MathUtil.applyDeadband(axFwd, Constants.Comp.Joystick.DEADBAND_LINEAR), 3))
				* Constants.Prop.Drivetrain.AXIS_SPEED_MAX;
		SmartDashboard.putNumber("input", axFwd);
		SmartDashboard.putNumber("new input", speedFwd);
		double speedStrafe = yLimiter
				.calculate(Math.pow(MathUtil.applyDeadband(axStrafe, Constants.Comp.Joystick.DEADBAND_LINEAR), 3))
				* Constants.Prop.Drivetrain.AXIS_SPEED_MAX;
		double speedRot = rotLimiter
				.calculate(Math.pow(MathUtil.applyDeadband(axRot, Constants.Comp.Joystick.DEADBAND_ANG), 3))
	...
	public double[] joystickFiltering(double x, double y, double rot) {
		...
		double speedFwd = xLimiter
				.calculate(Math.pow(MathUtil.applyDeadband(x, Constants.Comp.Joystick.DEADBAND_LINEAR), 3));
		double speedStrafe = yLimiter
				.calculate(Math.pow(MathUtil.applyDeadband(y, Constants.Comp.Joystick.DEADBAND_LINEAR), 3));
		double speedRot = rotLimiter
				.calculate(Math.pow(MathUtil.applyDeadband(rot, Constants.Comp.Joystick.DEADBAND_ANG), 3));
